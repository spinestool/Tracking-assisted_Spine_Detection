...
sliceno = iprob;
for ka=1:sliceno %load all the procesed images with their centroids desc locs
...
for i = 1:size(current_centroid,1)
    if Loc_Registration_swich_on==1
        ...
        current_centroid_new(i,:) = current_centroid(i,:)+[x_shift y_shift]+[U_1(round(current_centroid(i,1)),round(current_centroid(i,2))) U_2(round(current_centroid(i,1)),round(current_centroid(i,2)))];
    else
        current_centroid_new(i,:) = current_centroid(i,:)+[x_shift y_shift];
    end
end

current_centroid = current_centroid_new;
current_centroid_new = [];

[m1,n1]=size(current_centroid);
[m2,n2]=size(new_centroid);
...
for ia=1:m1 %t time frame
    r0=current_centroid(ia,:); %takes iath spine's center coordinates of first image
for ib=1:m2 %t+1 time frame
      r1=new_centroid(ib,:); %takes ibth spine's center coordinates of second image
      dist=sqrt((r1(:,1)-r0(:,1))^2+(r1(:,2)-r0(:,2))^2); %finds euclidean distance between spine centers of new image and previous image
      dist_vect(1,ia)=dist; %stores distances of each spines
      dist_history(ib,ia) = dist_vect(1,ia);
  
      if dist<=d_shift+5%Dist_Thresh %distance threshold
        dist_prev = dist_history_10(:,ia);
%       if dist<35 %distance threshold = 23 for first dataset. 35 is for 7th dataset
        if ib >1  
%keyboard
...
checkvec = 1:max(size(new_centroid,1));
    ...
    if lost_spines(step).h ~=0
        for ad = 1:size(lost_spines(step).h,2)
            ...
            if step-count2 == 1 %if that spine was appeared only at first time point
               new_cc(lost_spines(step).h(ad),:) = blobcentroid_all(step-count2).h(lost_spines(step).h(ad),:); 
            else
               new_cc(lost_spines(step).h(ad),:) = blobcentroid_all(step-count2).h(revised_labels(step-count2).h(find(revised_labels(step-count2).h(:,2) == lost_spines(step).h(ad)),1),:);
            end
            if count2 >1
                sum = 0;
                ...
                new_cc(lost_spines(step).h(ad),:) = new_cc(lost_spines(step).h(ad),:)+sum;
            else if count2 == 1 
%                     && step == sliceno
                    new_cc(lost_spines(step).h(ad),:) = new_cc(lost_spines(step).h(ad),:)+change_history(step).h;
                end
            end
        end
    end
    ...
    if lost_spines(step).h ~=0
        for ad = 1:size(lost_spines(step).h,2)
            new_cc(lost_spines(step).h(ad),:) = blobcentroid_all(step-1).h(lost_spines(step).h(ad),:)+change_history(step).h;
        end
    end
end

current_centroid = new_cc;
revised_centroids(step).h = current_centroid;
